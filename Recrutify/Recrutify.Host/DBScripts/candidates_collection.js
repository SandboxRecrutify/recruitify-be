var candidates = [
    {
        "_id": UUID("c07e12be-30bd-11ec-853c-0392a821ec1f"),
        "Name": "Christopher",
        "Surname": "Elliott",
        "EnglishLevel": NumberInt(0),
        "ProjectLanguage" : NumberInt(0),
        "PhoneNumber": "+375333421342",
        "Email": "christopherelliott558@gmail.com",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242344"
            },
            {
                "Type": "Instagram",
                "Value": "@bigbaby"
            }
        ],
        "Location": {
            "City": "Minsk",
            "Country": "Belarus"
        },
        "RegistrationDate": new Date("2021-08-04T00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(14), NumberInt(15), NumberInt(20)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("dd49fe8f-d697-41a5-be23-7066563b85a6"),
                "Feedbacks": [
                    {
                        "UserName": "Christopher Johnson",
                        "UserId": UUID("d1b13d9f-3799-456b-87e2-1bef7db71cb1"),
                        "Type": NumberInt(2),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(2),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(3),
                "PrimarySkill": {
                        "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                        "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            },
            {
                "ProjectId": UUID("bdbc3af0-f882-4f4f-bbb8-3e87ba16b768"),
                "Feedbacks": [
                    {
                        "Type": NumberInt(0),
                        "Rating": NumberInt(4),
                        "CreatedOn": new Date("2021-03-12T10:17:36.000Z")
                    },
                    {
                        "UserName": "Justin Walker",
                        "UserId": UUID("71da4db7-36ec-4a6b-bfb3-38d2462bdf2d"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(5),
                        "TextFeedback": "Able to learn, has a good basic knowledge",
                        "CreatedOn": new Date("2021-04-18T10:25:12.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("c89c4d88-30e7-11ec-929c-0f374fa83b31"),
        "Name": "Robert",
        "Surname": "Bruce",
        "EnglishLevel": NumberInt(2),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375337349087",
        "Email": "brucer295@gmail.com",
        "Contacts": [
                {
                    "Type": "Facebook",
                    "Value": "id=100002956189257"
                }
            ],
        "Location": {
                "City": "Moscow",
                "Country": "Russian Federation"
            },
        "RegistrationDate": new Date("2021-08-02T00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(21), NumberInt(22)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("dd49fe8f-d697-41a5-be23-7066563b85a6"),
                "Feedbacks": [
                    {
                        "UserName": "Fred Taylor",
                        "UserId": UUID("cb40d31e-7447-4ec0-bc53-ab566f3a7b2e"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(4),
                        "TextFeedback": "It is worth considering the admission to the project",
                        "CreatedOn": new Date("2021-08-13T12:10:30.000Z")
                    },
                    {
                        "Type": NumberInt(0),
                        "Rating": NumberInt(5),
                        "CreatedOn": new Date("2021-08-28T11:24:43.000Z")
                    },
                    {
                        "UserName": "Christopher Johnson",
                        "UserId": UUID("d1b13d9f-3799-456b-87e2-1bef7db71cb1"),
                        "Type": NumberInt(3),
                        "Rating": NumberInt(4),
                        "TextFeedback": "Demonstrated good knowledge of async programming.",
                        "CreatedOn": new Date("2021-09-28T10:57:34.000Z")
                    }
                ],
                "Status": NumberInt(5),
                "PrimarySkill": {
                    "_id": UUID("d66d9d10-3775-11ec-9f7d-f7d443b3f4f3"),
                    "Name": "BusinessAnalyst"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "Cafe Mint",
        "Certificates": "Oracle Certified",
        "AdditionalInfo": "Ready to start anytime"
    },
    {
        "_id": UUID("7d4ab133-4c7b-47c2-a7c6-827bdd010aae"),
        "Name": "Byron",
        "Surname": "Jackson",
        "EnglishLevel": NumberInt(2),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375332563489",
        "Email": "byronjackson147@gmail.com",
        "Contacts": [
            {
                "Type": "Instagram",
                "Value": "@jackgreen"
            },
            {
                "Type": "Facebook",
                "Value": "id=100002979189885"
            }
        ],
        "Location": {
            "City": "Vitebsk",
            "Country": "Belarus"
        },
        "RegistrationDate": new Date("2021-08-08T00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(8), NumberInt(9), NumberInt(18), NumberInt(19), NumberInt(20)],
        "GoingToExadel": false,
        "ProjectResults": [
            {
                "ProjectId": UUID("dd49fe8f-d697-41a5-be23-7066563b85a6"),
                "Feedbacks": [
                    {
                        "UserName": "Christopher Johnson",
                        "UserId": UUID("d1b13d9f-3799-456b-87e2-1bef7db71cb1"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(4),
                        "TextFeedback": "Fully suited to the project",
                        "CreatedOn": new Date("2021-08-16T10:27:18.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("d66d9d10-3775-11ec-9f7d-f7d443b3f4f3"),
                    "Name": "BusinessAnalyst"
                },
                "IsAssignedOnInterview": false
            },
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Status": NumberInt(0),
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LLC Acronis",
        "Certificates": "CompTIA",
        "AdditionalInfo": "Ready to start now"
    },
    {
        "_id": UUID("1e6a306e-30ec-11ec-a7bb-37cfa4f75a73"),
        "Name": "Brian",
        "Surname": "Gaines",
        "EnglishLevel": NumberInt(2),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375255486325",
        "Email": "gainesbrian05@mail.com",
        "Location": {
            "City": "Kiev",
            "Country": "Ukraine"
        },
        "RegistrationDate": new Date("2021-03-07T00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(14), NumberInt(15), NumberInt(16), NumberInt(17), NumberInt(18)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("bdbc3af0-f882-4f4f-bbb8-3e87ba16b768"),
                "Feedbacks": [
                    {
                        "UserName": "Justin Walker",
                        "UserId": UUID("71da4db7-36ec-4a6b-bfb3-38d2462bdf2d"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(3),
                        "TextFeedback": "Has basic skills and knowledge",
                        "CreatedOn": new Date("2021-03-14T10:11:14.000Z")
                    },
                    {
                        "Type": NumberInt(0),
                        "Rating": NumberInt(2),
                        "CreatedOn": new Date("2021-04-07T12:09:53.000Z")
                    },
                    {
                        "UserName": "Christopher Johnson",
                        "UserId": UUID("d1b13d9f-3799-456b-87e2-1bef7db71cb1"),
                        "Type": NumberInt(3),
                        "Rating": NumberInt(4),
                        "TextFeedback": "Fully suitable for independent work",
                        "CreatedOn": new Date("2021-04-29T11:43:50.000Z")
                    }
                ],
                "Status": NumberInt(5),
                "PrimarySkill": {
                    "_id": UUID("b7d88aa4-3775-11ec-93ad-f7d63f513914"),
                    "Name": "Java"
                },
                "IsAssignedOnInterview": false
            },
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Status": NumberInt(0),
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LLC Pilot",
        "AdditionalInfo": "I have experience in programming in js",
    },
    
    {
       "_id": UUID("37982a0e-30ed-11ec-a640-0f587540e9a4"),
       "Name": "Hugh",
       "Surname": "Harvey",
        "EnglishLevel": NumberInt(4),
        "ProjectLanguage": NumberInt(0),
       "PhoneNumber": "+375448536723",
        "Email": "harvey_hugh@mail.ru",
       "Location": {
           "City": "Grodno",
           "Country": "Belarus"
       },
       "RegistrationDate": new Date("2021-08-08T00:00:00.000Z"),
       "BestTimeToConnect": [NumberInt(14), NumberInt(15), NumberInt(16), NumberInt(20), NumberInt(21)],
       "GoingToExadel": true,
       "ProjectResults": [
           {
               "ProjectId": UUID("dd49fe8f-d697-41a5-be23-7066563b85a6"),
               "Feedbacks": [
                   {
                       "UserName": "Christopher Johnson",
                       "UserId": UUID("d1b13d9f-3799-456b-87e2-1bef7db71cb1"),
                       "Type": NumberInt(2),
                       "Rating": NumberInt(3),
                       "TextFeedback": "Has the basics of .Net 5.0",
                       "CreatedOn": new Date("2021-08-13T10:18:34.000Z")
                   },
                   {
                       "Type": NumberInt(0),
                       "Rating": NumberInt(3),
                       "CreatedOn": new Date("2021-08-26T12:14:11.000Z")
                   },
                   {
                       "UserName": "Fred Taylor",
                       "UserId": UUID("cb40d31e-7447-4ec0-bc53-ab566f3a7b2e"),
                       "Type": NumberInt(3),
                       "Rating": NumberInt(4),
                       "TextFeedback": "Fully owns the knowledge required for the project",
                       "CreatedOn": new Date("2021-09-22T11:12:53.000Z")
                   }
               ],
               "Status": NumberInt(5),
               "PrimarySkill": {
                   "_id": UUID("a539cb10-3775-11ec-8803-1b868ded28bd"),
                   "Name": "JavaScript"
               },
               "IsAssignedOnInterview": false
           },
           {
               "ProjectId": UUID("871b1e7a-30b5-11ec-9b40-437a6473123c"),
               "Status": NumberInt(0),
               "PrimarySkill": {
                   "_id": UUID("a9962438-3775-11ec-8787-6b8ecd0a876a"),
                   "Name": "AutomationQA",
               },
               "IsAssignedOnInterview": false
           }
       ],
       "CurrentJob": "PLC CosmosTV",
       "AdditionalInfo": "Always available for calls"
    },
    {
        "_id": UUID("cc85a012-5d25-41a0-bbb7-95a6403a6296"),
        "Name": "Anthony",
        "Surname": "Day",
        "EnglishLevel": NumberInt(2),
        "ProjectLanguage": NumberInt(0),
        "PhoneNumber": "+375443468923",
        "Email": "anthony_day23@mail.ru",
        "Contacts": [
            {
                "Type": "Facebook",
                "Value": "id=100002956147957"
            }
        ],
        "Location": {
            "City": "Moscow",
            "Country": "Russian Federation"
        },
        "RegistrationDate": new Date("2021-02-09T00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(6), NumberInt(7), NumberInt(13), NumberInt(14), NumberInt(15), NumberInt(20), NumberInt(21), NumberInt(22)],
        "GoingToExadel": false,
        "ProjectResults": [
            {
                "ProjectId": UUID("7d85284c-30b5-11ec-95cb-230b32afd221"),
                "Status": NumberInt(0),
                "PrimarySkill": {
                    "_id": UUID("d66d9d10-3775-11ec-9f7d-f7d443b3f4f3"),
                    "Name": "BusinessAnalyst"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LLC Huandai",
        "Certificates": "Oracle Certified"
    },
    {
        "_id": UUID("d66d9d10-3775-11ec-9f7d-f7d443b3f4f3"),
        "Name": "Yan",
        "Surname": "Mey",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375446768923",
        "Email": "yan@mail.ru",
        "Contacts": [
            {
                "Type": "Skype",
                "Value": "id=100002956147957"
            }
        ],
        "Location": {
            "City": "Boston",
            "Country": "USA"
        },
        "RegistrationDate": new Date("2021-02-09T00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(7), NumberInt(13), NumberInt(14), NumberInt(15)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("dd49fe8f-d697-41a5-be23-7066563b85a6"),
                "Status": NumberInt(3),
                "PrimarySkill": {
                    "_id": UUID("a539cb10-3775-11ec-8803-1b868ded28bd"),
                    "Name": "JavaScript"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LLC Huandai",
        "Certificates": "Oracle Certified"
    },
    {
        "_id": UUID("a539cb10-3775-11ec-8803-1b868ded28bd"),
        "Name": "Kiril",
        "Surname": "Pupkin",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(0),
        "PhoneNumber": "+375446761113",
        "Email": "pupkin@mail.ru",
        "Contacts": [
            {
                "Type": "Skype",
                "Value": "id=100002956147957"
            }
        ],
        "Location": {
            "City": "Moscow",
            "Country": "Russian Federation"
        },
        "RegistrationDate": new Date("2021-02-09T00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(7),NumberInt(15)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("871b1e7a-30b5-11ec-9b40-437a6473123c"),
                "Status": NumberInt(4),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LLC Huandai",
        "Certificates": "Oracle Certified"
    },
    {
        "_id": UUID("76a7567a-3564-4980-b3b9-afb1e1469df4"),
        "Name": "Victor",
        "Surname": "Tsoy",
        "EnglishLevel": NumberInt(0),
        "ProjectLanguage": NumberInt(0),
        "PhoneNumber": "+375335468978",
        "Email": "tsoy@gmail.com",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242344"
            },
            {
                "Type": "Skype",
                "Value": "v.tsoy"
            },
            {
                "Type": "Instagram",
                "Value": "@https://www.instagram.com/anatoliytsoy_official/?hl=ru"
            }
        ],
        "Location": {
            "City": "St. Petersburg",
            "Country": "Russia"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(14), NumberInt(15), NumberInt(20)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("dd49fe8f-d697-41a5-be23-7066563b85a6"),
                "Feedbacks": [
                    {
                        "UserName": "Brandon Harris",
                        "UserId": UUID("6f885f2c-b60d-4743-b381-4e841d48a956"),
                        "Type": NumberInt(2),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(2),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("d66d9d10-3775-11ec-9f7d-f7d443b3f4f3"),
                    "Name": "BusinessAnalyst"
                },
                "IsAssignedOnInterview": true
            },
            {
                "ProjectId": UUID("871b1e7a-30b5-11ec-9b40-437a6473123c"),
                "Feedbacks": [
                    {
                        "Type": NumberInt(0),
                        "Rating": NumberInt(4),
                        "CreatedOn": new Date("2021-03-12T10:17:36.000Z")
                    },
                    {
                        "UserName": "Fred Taylor",
                        "UserId": UUID("cb40d31e-7447-4ec0-bc53-ab566f3a7b2e"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(5),
                        "TextFeedback": "Able to learn, has a good basic knowledge",
                        "CreatedOn": new Date("2021-04-18T10:25:12.000Z")
                    }
                ],
                "Status": NumberInt(5),
                "PrimarySkill": {
                    "_id": UUID("a9962438-3775-11ec-8787-6b8ecd0a876a"),
                    "Name": "AutomationQA"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("bb1f336e-de7b-4f3e-ba27-771ce95d9b03"),
        "Name": "Miron",
        "Surname": "Fedorov",
        "EnglishLevel": NumberInt(0),
        "ProjectLanguage": NumberInt(0),
        "PhoneNumber": "+375336908654",
        "Email": "mf@mail.ru",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242844"
            },
            {
                "Type": "Skype",
                "Value": "m.f"
            }
        ],
        "Location": {
            "City": "St. Petersburg",
            "Country": "Russia"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(13), NumberInt(15), NumberInt(16)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("dd49fe8f-d697-41a5-be23-7066563b85a6"),
                "Feedbacks": [
                    {
                        "UserName": "Brandon Harris",
                        "UserId": UUID("6f885f2c-b60d-4743-b381-4e841d48a956"),
                        "Type": NumberInt(3),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(3),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("d66d9d10-3775-11ec-9f7d-f7d443b3f4f3"),
                    "Name": "BusinessAnalyst"
                },
                "IsAssignedOnInterview": true
            },
            {
                "ProjectId": UUID("871b1e7a-30b5-11ec-9b40-437a6473123c"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(0),
                        "Rating": NumberInt(3),
                        "CreatedOn": new Date("2021-03-12T10:17:36.000Z")
                    },
                    {
                        "UserName": "Fred Taylor",
                        "UserId": UUID("cb40d31e-7447-4ec0-bc53-ab566f3a7b2e"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(5),
                        "TextFeedback": "Able to learn, has a good basic knowledge",
                        "CreatedOn": new Date("2021-04-18T10:25:12.000Z")
                    }
                ],
                "Status": NumberInt(5),
                "PrimarySkill": {
                    "_id": UUID("a9962438-3775-11ec-8787-6b8ecd0a876a"),
                    "Name": "AutomationQA"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("5e0df651-2b85-4f6f-be03-9a4ef9719493"),
        "Name": "Nastya",
        "Surname": "Korshun",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375336908654",
        "Email": "n.korshun@mail.ru",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242844"
            },
            {
                "Type": "Skype",
                "Value": "n.korshun"
            }
        ],
        "Location": {
            "City": "Ivanovo",
            "Country": "Russia"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(13), NumberInt(15), NumberInt(16)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("bdbc3af0-f882-4f4f-bbb8-3e87ba16b768"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(2),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(4),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(7),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            },
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(3),
                        "TextFeedback": "Able to learn, has a good basic knowledge",
                        "CreatedOn": new Date("2021-03-12T10:17:36.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("02762043-a29a-4923-8734-e3789867a166"),
        "Name": "lina",
        "Surname": "Fedotova",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375336906654",
        "Email": "fedotova@mail.ru",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242844"
            },
            {
                "Type": "Skype",
                "Value": "fed.l"
            }
        ],
        "Location": {
            "City": "Volgograd",
            "Country": "Russia"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(10), NumberInt(11), NumberInt(16)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("bdbc3af0-f882-4f4f-bbb8-3e87ba16b768"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(2),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(5),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(7),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            },
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(2),
                        "Rating": NumberInt(5),
                        "TextFeedback": "Able to learn, has a good basic knowledge",
                        "CreatedOn": new Date("2021-03-12T10:17:36.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("3302ed0f-0820-4d48-a02c-08e4139de375"),
        "Name": "Kim",
        "Surname": "Kin",
        "EnglishLevel": NumberInt(0),
        "ProjectLanguage": NumberInt(0),
        "PhoneNumber": "+375336989654",
        "Email": "kimkin@mail.ru",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242844"
            },
            {
                "Type": "Skype",
                "Value": "fkim.1"
            }
        ],
        "Location": {
            "City": "Tashkent",
            "Country": "Russia"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(10), NumberInt(11), NumberInt(12), NumberInt(13), NumberInt(14)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("bdbc3af0-f882-4f4f-bbb8-3e87ba16b768"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(3),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(5),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            },
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(4),
                        "Rating": NumberInt(5),
                        "TextFeedback": "Able to learn, has a good basic knowledge",
                        "CreatedOn": new Date("2021-03-12T10:17:36.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": false
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("9ad565ca-5418-11ec-bf63-0242ac130002"),
        "Name": "Milk",
        "Surname": "Mac",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375336569654",
        "Email": "milk@mail.ru",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242844"
            },
            {
                "Type": "milk.1"
            }
        ],
        "Location": {
            "City": "Minsk",
            "Country": "Belarus"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(10), NumberInt(11), NumberInt(12), NumberInt(15), NumberInt(16)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(2),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(5),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("300a8724-5419-11ec-bf63-0242ac130002"),
        "Name": "Max",
        "Surname": "Mali",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375336569654",
        "Email": "mily@mail.ru",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242844"
            },
            {
                "Type": "max.1"
            }
        ],
        "Location": {
            "City": "Minsk",
            "Country": "Belarus"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(11), NumberInt(12), NumberInt(13), NumberInt(15), NumberInt(16)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(4),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(4),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("134ed6e9-7e44-4a44-ac2e-a9e89da78a01"),
        "Name": "Nuk",
        "Surname": "Boby",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375336569654",
        "Email": "mily@mail.ru",
        "Contacts": [
            {
                "Type": "VK",
                "Value": "id3242844"
            },
            {
                "Type": "boby.1"
            }
        ],
        "Location": {
            "City": "Minsk",
            "Country": "Belarus"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(13), NumberInt(15), NumberInt(16)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("2beed73a-30b5-11ec-808a-fb45776a1ed3"),
                "Feedbacks": [
                    {
                        "UserName": "Anthony Clark",
                        "UserId": UUID("73e05563-6c9e-4727-9ebb-b08f16cc1001"),
                        "Type": NumberInt(3),
                        "TextFeedback": "Weak knowledge of the database",
                        "Rating": NumberInt(3),
                        "CreatedOn": new Date("2021-08-15T11:45:17.000Z")
                    }
                ],
                "Status": NumberInt(1),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            }
        ],
        "CurrentJob": "LTD Bank-Alfa",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
    {
        "_id": UUID("bca021ca-7c57-428e-827e-80276cec6e13"),
        "Name": "Alesia",
        "Surname": "Lis",
        "EnglishLevel": NumberInt(1),
        "ProjectLanguage": NumberInt(1),
        "PhoneNumber": "+375291191293",
        "Email": "lis.alesia@mail.ru",
        "Contacts": [
            {
                "Type": "Skype",
                "Value": "live:.cid.340d5da4b77520a7"
            }
        ],
        "Location": {
            "City": "Minsk",
            "Country": "Belarus"
        },
        "RegistrationDate": new Date("2021-12-02 00:00:00.000Z"),
        "BestTimeToConnect": [NumberInt(10), NumberInt(11), NumberInt(12), NumberInt(13), NumberInt(14)],
        "GoingToExadel": true,
        "ProjectResults": [
            {
                "ProjectId": UUID("69b2fc0a-2a75-4b06-8e12-d72243607fc2"),
                "Feedbacks": [],
                "Status": NumberInt(0),
                "PrimarySkill": {
                    "_id": UUID("aeb0e468-3774-11ec-83d4-97dbf3c3f8eb"),
                    "Name": ".Net"
                },
                "IsAssignedOnInterview": true
            }
        ],
        "CurrentJob": "Cosmos TV",
        "AdditionalInfo": "I can only do an internship in the evening"
    },
]

